...
#ifdef HAS_PIR
// Called by pir_sensor when motion is detected
void onPirTriggered() {
  if (!foundCoordinator) return;
  char payload[48];
  snprintf(payload, sizeof(payload), "N:%s,PIR:1", NODE_NAME);
  mesh.send(coordinatorMac, MESH_TYPE_DATA, 0x02, (const uint8_t*)payload, strlen(payload), 4);
  ...
  if (packet->type == MESH_TYPE_DATA && directToMe) {
    ...
    if (len >= 4 && strncmp(msg, "cmd:", 4) == 0) {
      ...
      if (fromCoordinator) {
        ...
        char ack[82];  // "command received:" (17) + command (up to 60) + null
